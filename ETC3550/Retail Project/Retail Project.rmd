...
progressr::with_progress(
    trained_arimamodel <- training %>% model(
        manual1 =   ARIMA(box_cox(Turnover, lambda) ~ 1 + pdq(3, 0, 0) + PDQ(0, 1, 3)), # nolint
        manual1v2 = ARIMA(box_cox(Turnover, lambda) ~ 0 + pdq(3, 0, 0) + PDQ(0, 1, 3)), # nolint
        manual2 =   ARIMA(box_cox(Turnover, lambda) ~ 1 + pdq(2, 0, 0) + PDQ(0, 1, 3)), # nolint
        manual2v2 = ARIMA(box_cox(Turnover, lambda) ~ 0 + pdq(2, 0, 0) + PDQ(0, 1, 3)), # nolint
        manual3 =   ARIMA(box_cox(Turnover, lambda) ~ 1 + pdq(2, 0, 1) + PDQ(0, 1, 3)), # nolint
        manual3v2 = ARIMA(box_cox(Turnover, lambda) ~ 0 + pdq(2, 0, 1) + PDQ(0, 1, 3)), # nolint
        manual4 =   ARIMA(box_cox(Turnover, lambda) ~ 1 + pdq(3, 0, 0) + PDQ(1, 1, 2)), # nolint
        manual4v2 = ARIMA(box_cox(Turnover, lambda) ~ 0 + pdq(3, 0, 0) + PDQ(1, 1, 2)), # nolint
        manual5 =   ARIMA(box_cox(Turnover, lambda) ~ 1 + pdq(2, 0, 0) + PDQ(1, 1, 2)), # nolint
        manual5v2 = ARIMA(box_cox(Turnover, lambda) ~ 0 + pdq(2, 0, 0) + PDQ(1, 1, 2)) # nolint
    )
)
...
### ARIMA
```{r}
trained_etsmodel %>% select(-State, -Industry) %>% pivot_longer(everything(),
    names_to = "Model name",
    values_to = "Orders"
)
trained_arimamodel %>% select(-State, -Industry) %>% pivot_longer(everything(),
    names_to = "Model name",
    values_to = "Orders"
)
```
...
# trained_arimamodel %>%
#     forecast(h = "2 years") %>%
#     autoplot() + autolayer(myseries %>% filter_index("Dec 2016" ~ .))
    # autoplot(myseries)


augment(trained_arimamodel) %>%
