...
After a first order differencing, the hypothesis test gives a p value of 0.1 which is greater than 0.05 which means the null hypothesis is not rejected. Thus making the first difference of the dataset, stationary. 
...
progressr::with_progress(
    trained_etsmodel <- training %>%
        model(
            ...
            Transformed = ETS(box_cox(Turnover, ets_lambda) ~ error("A") +
                trend("A") + season("A"))
        )
)

trained_etsmodel %>% select(Original) %>% gg_tsresiduals()
trained_etsmodel %>% select(Transformed) %>% gg_tsresiduals()
```

```{r ARIMA Differencing}
training %>%
    mutate(diff_turnover = difference(Turnover, differences = 1)) %>%
...
```{r}
progressr::with_progress(
    trained_arimamodel <- myseries %>% model(
        auto = ARIMA(Turnover),
        manual = ARIMA(Turnover ~ pdq(1, 1, 1) + PDQ(2, 1, 2))
    )
)

trained_arimamodel %>% select(auto) %>% gg_tsresiduals()
trained_arimamodel %>% select(manual) %>% gg_tsresiduals()
```