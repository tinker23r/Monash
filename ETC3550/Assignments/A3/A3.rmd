...
```
<!-- Analysis -->
Using Guerrero's method, the log likelihood has a maxmium value at around 0.0845. Substituting lambda with 0.0845 in the box-cox transformation gives a similar plot to the previous log transformation. 
...
myseries %>%
    model(
        STL(Turnover ~ trend(window = 13) + season(window = "periodic"),
            robust = TRUE
        )
    ) %>%
    components() %>%
